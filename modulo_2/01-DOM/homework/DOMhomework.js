...
function displayToDos() {
  ...
  buildToDos(toDoItems).forEach(item => {
    toDoContainer.insertBefore(item, toDoContainer.firstElementChild);
  });
}
...
function addToDo() {
  ...
  const toDo = new ToDo(inputToDo.value.trim());
  toDoItems.push(toDo);
...
// Tu código acá:
const btnAddToDo = document.querySelector('#addButton');
...
function completeToDo(event) {
  ...
  // Tu código acá:
  const val = toDoItems[index].complete;
  toDoItems[index].complete = !val;
